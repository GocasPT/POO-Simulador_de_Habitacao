...
void Zona::update() {
    ...
    for (const auto &componente: componentes) {
        if (componente.second->getLetterID() == 'a')
            aparelhos.push_back((Aparelho *) componente.second);
        else if (componente.second->getLetterID() == 'p')
            processadores.push_back((Processador *) componente.second);
    }
